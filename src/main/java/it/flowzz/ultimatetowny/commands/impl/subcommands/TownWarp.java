...
public class TownWarp extends SubCommand {

    public TownWarp() {
        super("warp", "/town warp [town]");
    ...
    public void execute(CommandSender sender, String label, String[] args) {
        if (sender instanceof Player player) {
            if (args.length < 1) {
                ...
                plugin.getMenuHandler().getWarpMenu().open(player);
            } else {
                ...
                player.teleport(town.getWarp());
                player.sendMessage(Messages.WARPED_TO_TOWN.getTranslation().replace("%town%", town.getName()));
            }
        } else {
